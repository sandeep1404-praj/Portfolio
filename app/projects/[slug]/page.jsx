...
export default function ProjectDetail() {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-black text-zinc-900 dark:text-zinc-100 font-sans pb-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-16 pt-20 md:pt-32">
        {/* Top Navigation */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 md:mb-12">
          ...
          <div className="w-fit bg-zinc-100 dark:bg-zinc-800/50 border border-zinc-200 dark:border-zinc-700/50 px-4 py-1.5 rounded-md text-sm font-medium text-zinc-500 dark:text-zinc-400">
            {project.year}
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8 mb-12">
          ...
          {project.externalLink && (
             <a 
              ...
              className="w-full md:w-auto px-8 py-3 rounded-full border border-black dark:border-white hover:bg-black hover:text-white dark:hover:bg-white dark:hover:text-black transition-all duration-300 font-medium flex items-center justify-center gap-2"
            >
              Check it out
          ...
          {!project.externalLink && (
             <button className="w-full md:w-auto px-8 py-3 rounded-full border border-black dark:border-white hover:bg-black hover:text-white dark:hover:bg-white dark:hover:text-black transition-all duration-300 font-medium whitespace-nowrap">
              Check it out
            </button>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 mb-16 md:mb-20">
          <div className="lg:col-span-7">
            ...
            <div className="flex flex-wrap gap-3 mt-6 md:mt-10">
              {project.tags?.map((tag) => (
                <span key={tag} className="px-4 py-2 rounded-full bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-xs sm:text-sm font-medium text-zinc-600 dark:text-zinc-400">
                  {tag}
                </span>
              ))}
            </div>
          ...
          <div className="lg:col-span-5 flex flex-col gap-6 md:gap-8 lg:pl-10">
            {project.roles && (
        ...
        <div className="space-y-8 md:space-y-12 mb-20 md:mb-32">
          {project.video && (
             <div className="relative w-full aspect-video rounded-2xl md:rounded-3xl overflow-hidden border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900 flex items-center justify-center">
          ...
          {project.images?.map((img, idx) => (
            <div key={idx} className="relative w-full rounded-2xl md:rounded-3xl overflow-hidden border border-zinc-200 dark:border-zinc-800">
              <Image 
                src={img} 
          ...
          {!project.images && project.image && (
             <div className="relative w-full rounded-2xl md:rounded-3xl overflow-hidden border border-zinc-200 dark:border-zinc-800">
                <Image 
                  src={project.image} 
                  alt={project.title} 
                  width={1920} 
        ...
        <div className="space-y-20 md:space-y-32">
          ...
          {project.features && (
            <div>
              <h2 className="text-2xl md:text-3xl font-bold mb-6 md:mb-10 pb-4 border-b border-zinc-200 dark:border-zinc-800 text-black dark:text-white">Features</h2>
          ...
          {project.technologies && (
            <div>
              <h2 className="text-2xl md:text-3xl font-bold mb-6 md:mb-10 pb-4 border-b border-zinc-200 dark:border-zinc-800 text-black dark:text-white">Technologies used</h2>
              <ul className="space-y-6 md:space-y-8">
                {project.technologies.map((tech, i) => (
                  <li key={i} className="flex flex-col md:flex-row md:items-center gap-1 md:gap-4 group">
                    ...
                    <span className="text-sm sm:text-base text-zinc-600 dark:text-zinc-400">{tech.desc}</span>
                  </li>
                ))}
              </ul>
            </div>
          ...
          {project.buildSteps && (
            <div>
              <h2 className="text-2xl md:text-3xl font-bold mb-6 md:mb-10 pb-4 border-b border-zinc-200 dark:border-zinc-800 text-black dark:text-white">Build steps</h2>
              <div className="space-y-8 md:space-y-12">
                {project.buildSteps.map((step, i) => (
                  <div key={i} className="space-y-3 md:space-y-4">
                    <div className="flex gap-3 text-lg md:text-xl font-medium text-zinc-800 dark:text-zinc-300">
                      <span className="text-zinc-400 dark:text-zinc-500">{i + 1}.</span>
                      <span className="text-sm sm:text-base md:text-xl">{step.step}</span>
                    </div>
                    {step.command && (
                      <div className="relative group/code">
                        <div className="bg-zinc-50 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 p-4 md:p-6 rounded-xl font-mono text-xs sm:text-sm md:text-base text-zinc-600 dark:text-zinc-400 overflow-x-auto">
        ...
        {/* Footer Navigation */}
        <div className="mt-20 md:mt-40 pt-10 md:pt-20 border-t border-zinc-100 dark:border-zinc-900 flex flex-col md:flex-row justify-between items-center gap-10 md:gap-0">
            <Link 
              href={`/projects/${prevProject.slug}`}
              className="group flex flex-col items-start gap-4 p-4 md:p-6 rounded-2xl hover:bg-zinc-50 dark:hover:bg-zinc-900/50 transition-all border border-transparent hover:border-zinc-200 dark:hover:border-zinc-800 w-full md:w-auto md:min-w-[150px]"
            >
            ...
            <Link 
              ...
              className="group flex flex-col items-end gap-4 p-4 md:p-6 rounded-2xl hover:bg-zinc-50 dark:hover:bg-zinc-900/50 transition-all border border-transparent hover:border-zinc-200 dark:hover:border-zinc-800 w-full md:w-auto md:min-w-[150px] text-right order-2 md:order-3"
            >
              <div className="flex items-center gap-2 text-zinc-500 group-hover:text-black dark:group-hover:text-zinc-100">
